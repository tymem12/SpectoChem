# from https://github.com/PNNL-CompBio/ML_UVvisModels/blob/main/UVvis-MPNN/original_plus_spectra/chemprop/train/spectral_loss.py
...
def generate_conv_matrix(length: int = 181, stdev: float = 0.0, torch_device: str = 'cpu') -> torch.tensor:
...
def roundrobin_sid(spectra: torch.Tensor, threshold: float = 1e-8, torch_device: str = 'cpu', stdev: float = 0.0) -> torch.Tensor:
    ...
    for i in range(len(spectra)):
        with torch.no_grad():
            mol_spectra = spectra[i] #10*181
            nan_mask=torch.isnan(mol_spectra[0]) #181
            nan_mask=nan_mask.to(device=torch_device)
            ...
            mol_norm = torch.div(mol_spectra,mol_sums) #10*181
            mol_norm[:,nan_mask]=1
            ensemble_head = torch.zeros([0,spectrum_size],device=torch_device,dtype=float) #0*181
            ensemble_tail = torch.zeros([0,spectrum_size],device=torch_device,dtype=float) #0*181
            for j in range(len(mol_norm)-1):
                ensemble_tail = torch.cat((ensemble_tail,mol_norm[j+1:]),axis=0) #n*181
                ensemble_head = torch.cat((ensemble_head,mol_norm[:-j-1]),axis=0) #n*181
            mol_loss = torch.zeros_like(ensemble_head,device=torch_device,dtype=float) #n*181
